...
import { getPrivacy, getTerms } from '@/api/legal';
...
export default function PolicyModal({ visible, type, onClose }: PolicyModalProps) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const doc = type === 'terms' ? await getTerms() : await getPrivacy();
      ...
      setBody(
        type === 'terms'
          ? 'By using Selorg Rider you agree to follow delivery policies and use the app only for authorized work.'
          : 'We collect only the information required to operate rider services, attendance, and compliance workflows.'
      );
    } finally {
      setLoading(false);
    }
