/**
 * Aadhar Card Upload Screen Component
 * Dedicated screen for Aadhar card upload and verification
 * 
 * @component
 * @example
 * <AadharUploadScreen />
...
export default function AadharUploadScreen() {
  ...
  const [documentUri, setDocumentUri] = useState<string | null>(userData.aadharUri);
  const [error, setError] = useState('');
  const [uploading, setUploading] = useState(false);

  const validateAadhar = (id: string): boolean => {
    return /^\d{12}$/.test(id);
  };
  ...
  // Format Aadhar for display as "1234 5678 9012"
  ...
  const handleContinue = async () => {
    if (!aadharNumber && !documentUri) {
      setError('Please enter Aadhar number or scan the document');
      return;
    }
    ...
    if (documentUri) {
      ...
      setError('');
      try {
        const result = await uploadKycDocument(documentUri, 'aadhar', 'aadhar.jpg', 'image/jpeg', aadharNumber);
        const uploadedLink = (result && (result.uploadedLink || (result as any).uploadedLink)) ?? undefined;
        router.replace({ pathname: '/kyc-upload', params: { updatedDoc: 'aadhar', uploadedLink } });
      } catch (e) {
    ...
    router.replace({ pathname: '/kyc-upload', params: { aadharStatus: 'pending' } });
  };

  const isContinueEnabled = !uploading && ((aadharNumber.length === 12 && validateAadhar(aadharNumber)) || documentUri !== null);

  React.useEffect(() => {
    (async () => {
      const { setStoredOnboardingStep } = await import('@/api/storage');
      await setStoredOnboardingStep('/aadhar-upload');
    })();
  }, []);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.mainContainer}>
        <Header
          title="Aadhar Card"
          subtitle="Enter your Aadhar details or scan the card"
          onBack={handleBack}
        />
          ...
          <View style={styles.content}>
            ...
            <View style={styles.documentPreviewCard}>
              {documentUri ? (
                ...
                <View style={styles.documentPlaceholder}>
                  <DocumentAadharIcon size={scale(80)} color="#E5E7EB" />
                  <Text style={styles.placeholderText}>No document uploaded</Text>
                </View>
              )}
            </View>

            {/* Aadhar Number Input Section */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Aadhar Number</Text>
              ...
              <View style={styles.inputContainer}>
                <TextInput
                  ...
                  maxLength={14} // includes spaces for display
                />
              </View>
              ...
              <Text style={styles.helperText}>
                Enter your 12-digit Aadhar number as printed on the card
              </Text>
            </View>
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.sectionDescription}>
                Upload a clear photo of your Aadhar card
              </Text>
              ...
              >
                <Text style={styles.scanButtonText}>
                  {documentUri ? 'Change Document' : 'Scan Aadhar Card'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  inputContainer: {
    width: '100%',
    height: scale(56), // Increased from 49
    backgroundColor: Theme.colors.white,
    borderWidth: 1,
    borderColor: Theme.colors.borderGrey,
    borderRadius: 8,
