/**
 * Order Details Screen Component
 * General screen for viewing order details – now upgraded with real map, route, and functional UI
 */
...
export default function OrderDetailsScreen() {
  ...
  const rawPickupLocation = (order?.darkstoreCode || order?.warehouseCode) || params.pickupLocation || 'Hub';
  const pickupLocation = rawPickupLocation.startsWith('Darkstore') ? rawPickupLocation : `Darkstore ${rawPickupLocation}`;
  
  const pickupBay = params.pickupBay ?? '';
  ...
  }, [order?.darkstoreCode, order?.warehouseCode, rawPickupLocation, deliveryAddress?.city]);
  ...
  }, [orderId, deliveryLocation, deliveryAddress?.coordinates?.lat, deliveryAddress?.coordinates?.lng]);
  ...
  }, [riderLocation?.latitude, riderLocation?.longitude, hubCoords?.latitude, hubCoords?.longitude]);
  ...
  }, [hubCoords, hubAddress, deliveryLocation]);
  ...
  }, [orderId, router, pickupLocation, pickupBay, deliveryLocation, estimatedPayout, totalItems]);
    ...
    <SafeAreaView style={orderDetailsStyles.container} edges={['top', 'bottom']}>
      <View style={orderDetailsStyles.mapContainer}>
        ...
        <View style={orderDetailsStyles.mapOverlay} pointerEvents="box-none">
          <TouchableOpacity
            style={[orderDetailsStyles.topBackButton, { backgroundColor: Theme.colors.white, borderRadius: scale(22), ...Theme.shadows.small }]}
          ...
          <TouchableOpacity
            style={orderDetailsStyles.locationButton}
      ...
      >
        <Pressable
          style={orderDetailsStyles.sheetHandle}
        ...
        >
          <View style={[orderDetailsStyles.hubInfoSection, { flexWrap: 'nowrap' }]}>
            ...
            <View style={[orderDetailsStyles.hubInfo, { flex: 1, minWidth: 0 }]}>
              ...
              {pickupBay ? (
                <View style={orderDetailsStyles.dispatchBayBadge}>
                  <Text variant="bodySm" color={Theme.colors.primaryMedium} style={orderDetailsStyles.dispatchBayText}>
                    {pickupBay}
                  </Text>
                </View>
              ) : null}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: scale(10), flexShrink: 0 }}>
              <TouchableOpacity
                style={[orderDetailsStyles.callButton, { width: scale(48), height: scale(48), borderRadius: scale(24) }]}
                onPress={handleCall}
                activeOpacity={0.8}
              >
