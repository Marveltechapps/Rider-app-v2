/**
 * Earnings Month Screen Component
 * Earnings screen showing this month's earnings, active incentives, and payout history
...
export default function EarningsMonthScreen() {
  const router = useRouter();
  const activeTab: EarningsTab = 'thisMonth';

  const { data: summaryRes } = useQuery({
    queryKey: ['earnings', 'summary', 'month'],
    queryFn: () => getEarningsSummary('month'),
    staleTime: 60 * 1000,
  });
  ...
  const handleTabChange = useCallback(
    (tab: EarningsTab) => {
      ...
      // If tab is 'thisMonth', stay on current screen
    },
    [router],
  );
